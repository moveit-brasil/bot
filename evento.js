...
(function () {
	...
	var basicBot = {
		...
		settings: {
			...
			themeLink: ":flag-br:Tema Nacional: Livre (todos os gêneros brasileiros permitidos). Aqui é BR mas não é bagunça! (Zoeira somente das 22h às 07h)",
		   ...
		   roulettepp: {
			...
			roulettetroll: {
				...
				endRoulette: function () {
					...
					var posxtroll = Math.floor((Math.random() * API.getWaitList().length) + 1);
					...
					var name = user.username;
					API.sendChat(subChat(basicBot.chat.winnerpickedtroll, {name: name, positiontroll: posxtroll}));
			...
			msgCommand: {
				...
				functionality: function (chat, cmd) {
					...
					if (!basicBot.commands.executable(this.rank, chat)) return void (0);
					   ...
					   var c, mensagens;
					  mensagens = ["/me :flag-br:Tema Nacional: Livre (todos os gêneros brasileiros permitidos). Aqui é BR mas não é bagunça! (Zoeira somente das 22h às 07h)",
